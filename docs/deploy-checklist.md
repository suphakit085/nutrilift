...
- [ ] ส่ง `eval/reports/baseline-v10_expert.csv` ให้ผู้เชี่ยวชาญ/นักกำหนดอาหาร/เทรนเนอร์ให้คะแนน blind
      (v10 = คำตอบของ prompt v1.11.0 ที่จะ deploy จริง; v9 และก่อนหน้าเป็น prompt เก่า ห้ามส่ง)
      **ส่งไฟล์นี้ไฟล์เดียว** — `baseline-v10_expert_key.csv` คือไฟล์เฉลยที่แมปคอลัมน์ A/B กลับไปเป็น
      rag/no-RAG ถ้าผู้ให้คะแนนเห็น การ blind จะเสียทันทีและผลที่ได้ใช้อ้างอิงไม่ได้
      (ไฟล์ key ถูก gitignore ไว้แล้ว เก็บไว้ในเครื่องเท่านั้น) — ขั้นนี้ไม่ต้องรอ deploy
...
- [ ] เมื่อรวบรวมผลผู้เชี่ยวชาญ + SUS ครบ ค่อยรัน Wilcoxon signed-rank บนคะแนน**ของผู้เชี่ยวชาญ**
      (ฝั่ง LLM-judge รันไปแล้วบน baseline-v10: 150 คู่ correctness +0.22 p=0.0001)
