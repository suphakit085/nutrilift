...
**ที่ยังค้าง ต้องตัดสินใจก่อนสอบ (ไม่บล็อก deploy)**
- 10 แถวที่ source เป็น `TOVERIFY-INMU` / `TOVERIFY-LABEL` — ตัวเลขกลม ๆ ที่ตรงกับค่า USDA
  มากกว่าค่าไทย (อกไก่ต้ม 165 kcal / โปรตีน 31 g คือค่า USDA ของอกไก่สุก ขณะที่แถวที่มี source จริง
  `ไก่, อก, ดิบ` ให้โปรตีน 19.9 g) ระบบ**ไม่ได้โกหก**: แผนอาหารตัดแถวเหล่านี้ทิ้งทั้งหมด และ
  `lookup_food` ติดธง `estimated: true` พร้อมสั่งให้โมเดลบอกผู้ใช้ — แต่ในเชิงวิทยานิพนธ์ยังตอบไม่ได้ว่า
  ตัวเลขมาจากไหน ทางเลือก: หา source จริงมาอ้าง (USDA FDC ก็อ้างได้ ขอแค่ไม่อ้างว่าเป็น INMU)
  หรือลบทิ้ง — ต้องเลือกเอง เพราะกฎของโปรเจกคือห้ามแต่งแถวขึ้นมาเอง
- ชื่อ "แป้งมันสัมปะหลัง"/"มันสัมปะหลัง, ยอด" สะกดตาม PDF ต้นฉบับ (มาตรฐานคือ *สำ*ปะหลัง)
  ตอนนี้แถวแป้งถูกตัดออกแล้ว เหลือแถวยอดที่ยังสะกดตามต้นฉบับ
