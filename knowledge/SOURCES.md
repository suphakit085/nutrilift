...
## 3c. เอกสารใหม่ (4 ก.ย. 2569) — องค์ประกอบร่างกายแยกตามเพศ

หาเพิ่มเพราะ `goal_sex_personalization_eval` (3 ก.ย.) พบว่าระบบตอบคำถาม "ควรลดไขมันเหลือกี่ %"
ให้ผู้ใช้หญิงได้แค่หลักการกว้าง ๆ เนื่องจากไม่มีการ์ดใบไหนพูดถึงเรื่องนี้เลย ทั้งที่เป็นคำถามที่ตอบผิด
แล้วอันตรายจริง (ช่วง 10-12% ของผู้ชายต่ำกว่าเกณฑ์ขั้นต่ำของผู้หญิงในวงการกีฬาเสียอีก)

- [x] **Jagim AR, Tinsley GM, Oppliger RA, Horswill CA, Dobbs WC, Fields JB, Cushard C,
  Rademacher PD, Jones MT.** Collegiate women's wrestling body fat percentage and minimum
  wrestling weight values: time for revisiting minimal body fat percent?
  *J Int Soc Sports Nutr.* 2024;21(1):2304561. doi:10.1080/15502783.2024.2304561
  ✅ https://pmc.ncbi.nlm.nih.gov/articles/PMC10795649/ — **CC BY 4.0** (ตรวจใบอนุญาตจากหน้าบทความจริง)
  ✅ ไฟล์ `Jagim-2024-women-wrestling-minbf.pdf` (12 หน้า) — โหลดผ่าน Europe PMC
  (`https://europepmc.org/articles/PMC10795649?pdf=render`) เพราะ PMC บล็อกการโหลดตรงเหมือนเดิม
  ใช้สำหรับ: เกณฑ์ไขมันขั้นต่ำที่หน่วยงานกีฬาใช้จริง **ชาย 5% (มหาวิทยาลัย) / 7% (มัธยม) เทียบหญิง 12%**
  · ข้อมูลจริงนักมวยปล้ำหญิง 1,683 คน มัธยฐาน 27.4% เปอร์เซ็นไทล์ที่ 5 = 17% · ข้อเสนอให้ยกเกณฑ์
  หญิงจาก 12% → 17% · จุดสำคัญที่สุดคือ **ไขมันต่ำไม่ใช่สาเหตุหลัก** แต่เป็นการอยู่ในภาวะพลังงานต่ำ
  (<30 kcal/kg FFM) ต่อเนื่อง ซึ่งเชื่อมกับ EA/RED-S ในการ์ด `energy-balance-cut-bulk` พอดี
  → การ์ด `body-fat-by-sex` (ใช้ร่วมกับ ACSM 2016 หัวข้อ Body composition และ Helms 2014 ที่มีอยู่แล้ว)
...
| `energy` (เพิ่ม 4 ก.ย. 2569) | เปอร์เซ็นต์ไขมันที่เหมาะสมแยกตามเพศ ✅ (`body-fat-by-sex`) — ไม่ได้อยู่ในแผนเดิม เพิ่มเพราะ eval พบช่องว่าง |
